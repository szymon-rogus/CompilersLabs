...
class BinOperationUtils(object):
    def __init__(self):
...
class Interpreter():
    ...
    def visit(self, node):
        if self.assign_operators().__contains__(node.operator):
            desugarized = AST.Assignment(node.leftOperand,
                                         AST.BinOperation(node.leftOperand, self.assign_op_dict()[node.operator],
                                                          node.rightOperand, node.lineno), node.lineno)
    ...
    def visit(self, node):
        try:
            ...
            if (len(func.args) != len(args)):
                raise RuntimeError(node.name + "has wrong arguemnts")
            else:
                prev = self.memory
